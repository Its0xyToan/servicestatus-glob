...
async function testAllStatuses() {
    ...
    for (const e of allEntries) {
        const results: { name: string, status: string, components: number }[] = [];
        console.log(`${e.name} Status:`)

        for (const [key, GameClass] of e.entries) {
            try {
                // Instantiate the game class
                const gameInstance = new GameClass() as any;
                ...
                if (Array.isArray(components)) {
                    ...
                    if(components?.components) {
                        components?.components.forEach((component: any) => results.push({ name: "|> " + component.name, status: component.status, components: 0 }))
                    }
                }
